...
function buildContextSuggestion(transcriptPath, bucketFile, env) {
  try {
    ...
    return `[StrategicCompact] Context ~${approxTokens} tokens (${percent}% of ${formatWindowLabel(windowTokens)} window). Act now, in this response: tell the user this is a good point to compact, and propose a ready-to-run "/compact <focus instructions>" command. Per Claude Code's docs, the argument to /compact is a short FOCUS INSTRUCTION for what the automatic summary should prioritize keeping (e.g. "focus on the auth middleware rewrite, keep the API design decisions") — a phrase, not a pre-written narrative summary. Claude Code generates the actual summary from the real conversation; you are not writing that summary yourself. Do not just silently note this signal.`;
  } catch (err) {
...
async function main() {
  ...
  if (count === threshold) {
    messages.push(`[StrategicCompact] ${threshold} tool calls reached. If this looks like a phase boundary (not mid-implementation), act now: tell the user and propose a ready-to-run "/compact <focus instructions>" command — a short phrase telling the compactor what to prioritize (Claude Code generates the actual summary itself; you are not writing it). Do not just silently note this signal.`);
  } else if (count > threshold && (count - threshold) % 25 === 0) {
    messages.push(`[StrategicCompact] ${count} tool calls reached. If context looks stale for the current task, act now: tell the user and propose a ready-to-run "/compact <focus instructions>" command — a short phrase telling the compactor what to prioritize, not a pre-written summary. Do not just silently note this signal.`);
  }
